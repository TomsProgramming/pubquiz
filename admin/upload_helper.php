<?php
// Upload helper - centrale bestand upload logica met validatie
...
/**
 * Handle een geüpload bestand.
 *
 * @param array $file Entry uit $_FILES (of sub-array bij array inputs)
 * @param string $type UPLOAD_TYPE_* constante
 * @return array ['ok' => bool, 'path' => string|null, 'error' => string|null]
 *               path is relatief vanaf project root (bijv. "uploads/photos/xyz.jpg")
 */
function handle_upload($file, $type) {
    ...
    if ($file['error'] !== UPLOAD_ERR_OK) {
        ...
        // Hint als post_max_size kleiner is dan onze app-limiet
...
/**
 * Extract YouTube video ID uit een URL of losse ID.
 * Accepteert alleen strikte 11-char ID ([a-zA-Z0-9_-]).
 */
...
/**
 * Verwijder een geüpload bestand veilig (blijft binnen uploads/ directory).
 */
